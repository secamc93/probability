...
| 2026-09-07 | [502 en produccion: dos deploys pisandose el archivo de upstreams](2026-09-07-deploy-paralelo-tumbo-el-sitio.md) | El merge del PR #93 toco backend y frontend, los dos workflows corrieron en paralelo y reescribieron `active.conf` entero con 0,2 s de diferencia. Quedo `backend=blue` con blue ya apagado: nginx enrutando a un contenedor inexistente, 502 en todo `/api/` y `Unexpected token '<'` en el login. Corregido con `bg_switch`, que toma candado, relee el color del otro servicio dentro de la seccion critica y escribe con `mv` atomico. Reproducido y verificado con dash antes de desplegar |
| 2026-09-07 | [MYS-0879: guia salio a Cajica en vez de Bogota](2026-09-07-mys-0879-guia-cajica-siendo-bogota.md) | **TKT-000074**, en `testing`. `ResolveOrderGeozone` confiaba solo en coordenadas (ST_Contains); Google geocodifico la direccion dentro de Cajica y el texto correcto (Bogota) nunca se uso, porque `geozone_city_id` alimenta `destination_dane_code` que el frontend prioriza. Corregido con cross-check punto-vs-texto (gana el texto si discrepan), en orders y shipments. Verificado con 5 casos simulados y una orden real recreada en local. Pusheado a main (`cbeb0fe6`), falta verificar en produccion tras el deploy |
| 2026-09-07 | [WooCommerce: el stock de variantes nunca llegaba, se perdia en el consumer](2026-09-07-woocommerce-stock-variantes-se-pierde-en-consumer.md) | Reportado por Viga (sospechaba que un import con WebToffee le cambio el ID del producto), pero la causa real es anterior y afecta a TODO negocio con variantes en WooCommerce desde julio: el consumer de push de stock define su propia struct sin el campo `external_variant_id`, y Go descarta en silencio ese campo del JSON al deserializar. El PUT siempre pegaba al producto padre, que no controla stock en un producto variable. Corregido agregando el campo y componiendo `parent:variacion`. Test unitario nuevo (no habia cobertura). Sin desplegar, sin verificar E2E contra Woo real |
