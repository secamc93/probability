...
| 2026-09-07 | [MYS-0879: guia salio a Cajica en vez de Bogota](2026-09-07-mys-0879-guia-cajica-siendo-bogota.md) | Ticket pendiente de crear (bloqueo de acceso a produccion en la sesion). `ResolveOrderGeozone` confiaba solo en coordenadas (ST_Contains); Google geocodifico la direccion dentro de Cajica y el texto correcto (Bogota) nunca se uso, porque `geozone_city_id` alimenta `destination_dane_code` que el frontend prioriza. Corregido con cross-check punto-vs-texto (gana el texto si discrepan), en orders y shipments. Verificado con 5 casos simulados contra copia local (fixture sintetico). Sin desplegar, sin verificar contra produccion |
